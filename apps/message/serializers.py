...
class BookMessageThemeSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        #统计该主题下的回复数
        ret['comment_count']=Comment.objects.filter(message=instance,pre_comment=None).count()
        #楼中楼中楼内容
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        #统计该评论下的回复数
        ret['comment_count']=Comment.objects.filter(pre_comment=instance).count()
        #构造该评论下的回复树
        ret['comment_tree']=[]
        #一级回复，即对该评论的回复
...
class BookMessageThemeSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        ret['username'] = instance.user.username
        #统计该主题下的回复数
        ret['comment_count']=MpttComment.objects.filter(message=instance).count()
        #楼中楼中楼内容
...
class MpttCommentDetailSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        ret['childrens']=[]
        #增加以及评论下的二级评论
        childrens=list(instance.get_children().select_related('comment_user').order_by('updated_time').values('id',
                                                                          'comment_content',
